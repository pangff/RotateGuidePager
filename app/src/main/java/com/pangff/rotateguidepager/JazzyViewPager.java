...
public class JazzyViewPager extends ViewPager {
    ...
    PageAnimationChangeListener pageAnimationChangeListener;
    ...
    private static final float ROT_MAX = 60.0f;
    private static final float INIT_ROT_MAX = 240.0f;
    ...
    public interface PageAnimationChangeListener{
        public void onPageAnimationChanged(float dx);
    }

    public void setPageAnimationChangeListener(PageAnimationChangeListener pageAnimationChangeListener){
        this.pageAnimationChangeListener = pageAnimationChangeListener;
    }
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        int effect = ta.getInt(R.styleable.JazzyViewPager_style, 9);
     ...
     * @param up
     */
    private void animateRotateCircle(View left, View right, float positionOffset, boolean up) {
        if (mState != State.IDLE) {
            if (left != null) {
                View imageView = left.findViewById(R.id.imageView);
                manageLayer(left, true);
                mRot = (up ? 1 : -1) * (ROT_MAX * positionOffset);
                mTrans = (up ? -1 : 1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math.cos(mRot * Math.PI / 180.0f));
                ViewHelper.setPivotX(left, left.getMeasuredWidth() * 0.5f);
                ViewHelper.setPivotY(left, up ? 0 : left.getMeasuredHeight());
                ViewHelper.setTranslationY(imageView, mTrans);
                ViewHelper.setRotation(imageView, mRot);
                if (Math.abs(Math.abs(lastmRot) - Math.abs(mRot)) < 5) {
                    if(pageAnimationChangeListener!=null){
                        pageAnimationChangeListener.onPageAnimationChanged(mRot - lastmRot);
                    }
                }
                lastmRot = mRot;
            }
            if (right != null) {
                View imageView = right.findViewById(R.id.imageView);
                manageLayer(right, true);
                mRot = (up ? 1 : -1) * (-ROT_MAX + ROT_MAX * positionOffset);
                mTrans = (up ? -1 : 1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math.cos(mRot * Math.PI / 180.0f));
                ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0.5f);
...
//		mRight = getChildAt(position+1);
        ...
        switch (mEffect) {
            ...
            case RotateUp:
                ...
                break;
            case RotateDown:
                animateRotateCircle(mLeft, mRight, effectOffset, false);
                break;
            case Accordion:
